...
/**
 * Enhanced ResponsiveDebugger - AIRAVATA Edition
    ...
    <div className={`fixed ${positionClasses[position]} z-[9999] bg-black/95 text-white text-xs rounded-lg shadow-xl border border-gray-600 font-mono transition-all duration-300`}>
      <div className="p-3">
        <div className="flex items-center justify-between mb-2">
          <span className="font-bold text-blue-400">AIRAVATA Debug</span>
...
export default ResponsiveDebugger;
  ...
  return (
    <div className={`fixed ${positionClasses[position]} z-[9999] bg-black/90 text-white text-xs p-3 rounded-lg shadow-lg border border-gray-600 font-mono max-w-xs`}>
      <div className="flex items-center justify-between mb-2">
        <span className="font-bold text-blue-400">Responsive Debug</span>
        <button 
          onClick={() => setIsVisible(!isVisible)}
      ...
      {isVisible && (
        <div className="space-y-1">
          ...
          <div className="pt-2 border-t border-gray-700">
            <div className="text-gray-400 text-2xs">Active flags:</div>
            <div className="flex flex-wrap gap-1 mt-1">
              {responsive.isVerySmall && <span className="bg-red-600 px-1 rounded text-2xs">xxs</span>}
              {responsive.isSmall && <span className="bg-orange-600 px-1 rounded text-2xs">xs</span>}
              {responsive.isMobileXS && <span className="bg-yellow-600 px-1 rounded text-2xs">sm</span>}
              {responsive.isMobile && <span className="bg-green-600 px-1 rounded text-2xs">md</span>}
              {responsive.isTablet && <span className="bg-blue-600 px-1 rounded text-2xs">lg</span>}
              {responsive.isDesktop && <span className="bg-purple-600 px-1 rounded text-2xs">xl</span>}
              {responsive.isLargeDesktop && <span className="bg-pink-600 px-1 rounded text-2xs">2xl</span>}
            </div>
          </div>
          
          <div className="pt-2 border-t border-gray-700">
            <div className="text-gray-400 text-2xs">Test sizes:</div>
            <div className="flex flex-wrap gap-1 mt-1 text-2xs">
              <button 
                onClick={() => window.resizeTo(320, 568)}
                className="bg-gray-700 hover:bg-gray-600 px-1 rounded"
              >
                320px
              </button>
              <button 
